...
function CdnController(){
	...
	this._setCdnUrl = async function(newCdnUrl){
		// Change the CDN Url, for testing purposes
		cdnUrl = stripSlashes(newCdnUrl);
		await setClientCdnUrl(cdnUrl);
	}
	...
	if (validateSettings(rootUrl, cdnUrl)){
        setClientCdnUrl(cdnUrl).then();
		configureBrowserPolicy(cdnUrl);
		WebApp.rawHandlers.use(static404connectHandler);
		WebApp.rawHandlers.use(CORSconnectHandler);
